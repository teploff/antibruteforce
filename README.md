...
Разработан command-line интерфейс для ручного администрирования сервиса. Через CLI существует возможность вызвать сброс бакета и управлять whitelist/blacklist-ом. CLI может работает через HTTP интерфейс.
...
Для того, чтобы ограничить частоту попыток авторизации для данного логина, необходимо перейти в раздел конфигурации rate_limiter -> login. 
...
- ExpireTime - интервал времени, через который bucket считается протухшим. Величина так же соотвествует типу time.Duration

Конфигурация для password и ip представляет идентичную стркутуру.
